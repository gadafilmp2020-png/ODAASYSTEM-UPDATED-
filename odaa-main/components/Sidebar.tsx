...
export const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <>
      ...
      {/* FIXED: Positioned to the LEFT (left-4) to match the standard dashboard layout and prevent covering content */}
      <div className={`
        fixed top-4 left-4 bottom-4 w-72 bg-brand-surface flex flex-col border border-brand-lime/20 rounded-[3rem] z-50 transition-transform duration-500 backdrop-blur-3xl shadow-[0_20px_50px_rgba(0,0,0,0.5)]
      ...
      `}>
        ...
        <div className="px-6 py-6">
           <div className="p-5 rounded-[2rem] bg-black/20 border border-brand-lime/10 flex items-center gap-4 group hover:bg-black/40 transition-all">
        ...
        <nav className="flex-1 px-4 space-y-2 overflow-y-auto custom-scrollbar pt-2">
          <p className="px-5 text-[9px] font-black text-slate-600 uppercase tracking-[0.4em] mb-4 font-lora italic">Main Menu</p>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-5 w-full">
                  ...
                  <span className="text-[11px] font-black uppercase tracking-widest font-tech flex-1 text-left">{t(item.labelKey)}</span>
                </div>
                {item.id === 'ADMIN_DASHBOARD' && adminActionCount > 0 && (
                   <span className="absolute right-4 bg-rose-500 text-white text-[9px] px-2 py-0.5 rounded-lg font-black animate-pulse">{adminActionCount}</span>
                )}
              </button>
            );
          })}
        ...
        <div className="p-8 space-y-3 bg-black/20 rounded-b-[3rem] border-t border-white/5">
          ...
          <button 
            onClick={toggleTheme}
            className="w-full flex items-center justify-between px-5 py-3 rounded-2xl text-slate-500 hover:text-white hover:bg-white/5 transition-all group border border-transparent hover:border-white/5"
          >
            <div className="flex items-center gap-5 w-full">
          ...
          <button 
            onClick={onLogout}
            className="w-full flex items-center gap-5 px-5 py-4 rounded-2xl text-slate-500 hover:text-rose-500 hover:bg-rose-500/10 transition-all group"
          >
            <LogOut className="w-5 h-5 group-hover:-translate-x-1 transition-transform" />
            <span className="text-[11px] font-black uppercase tracking-widest font-tech flex-1 text-left">{t('logout')}</span>
          </button>
        </div>
      </div>
    </>
